...
* **Xét ý A (192.168.1.5/28):** Prefix = 28 -> Khóa cứng 28 bit đầu làm Network Part, còn lại 32 - 28 = 4 bit cuối là Host Part. Khai triển Octet thứ 4 (số 5) sang 8 bit nhị phân:
  * Số 5 = 0000.0101 -> Ghép nhãn Prefix 28 bit: [192.168.1.0000] [0101] (28 bit Net Part - 4 bit Host Part)
...
* **Xét ý B (192.168.1.16/29):** Prefix = 29 -> Khóa cứng 29 bit Net Part, còn lại 32 - 29 = 3 bit cuối làm Host Part. Khai triển Octet thứ 4 (số 16):
  * Số 16 = 0001.0000 -> Ghép nhãn Prefix 29 bit: [192.168.1.0001.0] [000] (29 bit Net Part - 3 bit Host Part)
...
* **Xét ý C (10.10.5.192/26):** Prefix = 26 -> Khóa cứng 26 bit Net Part, còn lại 32 - 26 = 6 bit cuối làm Host Part. Khai triển Octet thứ 4 (số 192):
  * Số 192 = 1100.0000 -> Ghép nhãn Prefix 26 bit: [10.10.5.11] [00.0000] (26 bit Net Part - 6 bit Host Part)
...
### 🛠️ Tiến trình xử lý:
1. **Tính tổng số lượng địa chỉ IP:** Prefix = 23 -> Số bit phần Host Part = 32 - 23 = 9 bit. Áp dụng công thức số trường hợp cấu thành, tổng số lượng IP nằm trong mạng con này là: 2^9 = **512 địa chỉ IP**.
2. **Tìm địa chỉ Network ID (IP Đầu dải số - Host bit = full 0):** Khai triển Octet thứ 3 (số 2) sang nhị phân: Số 2 = 0000.0010. Ghép nhãn khóa cứng 23 bit Net Part, gán 9 bit phần Host bằng số 0:
  * [192.168.0000.001] [0] . [0000.0000] (23 bit Net Part - 9 bit Host Part)
  * Gộp nhị phân về lại dạng thập phân từng Octet ➔ **Network ID: 192.168.2.0**.
3. **Tìm địa chỉ Broadcast IP (IP Cuối dải số - Host bit = full 1):** Giữ nguyên 23 bit vùng Net Part, gán kịch trần 9 bit phần Host bằng số 1:
  * [192.168.0000.001] [1] . [1111.1111] (23 bit Net Part - 9 bit Host Part)
  * Tính toán giá trị thập phân từng vùng:
    * Octet thứ 3: Cụm bit 0000.0011 -> Khai triển thành: 2^1 + 2^0 = 2 + 1 = **3**.
    * Octet thứ 4: Cụm 8 bit 1 kịch trần 1111.1111 -> Đạt giá trị tối đa là **255**.
...
### 🛠️ Tiến trình xử lý:
* **Bước 1:** Prefix = 22 -> Khóa cứng 22 bit làm vùng mạng. Ta phân rã Octet thứ 3 (số 10) sang 8 bit nhị phân để dò ranh giới cắt bit: Số 10 = 0000.1010.
* **Bước 2:** Ranh giới cắt bit 22 nằm khía tại vị trí bit số 2 của Octet thứ 3. Tiến hành khóa cứng hoàn toàn 22 bit đầu, chạy thuật toán tìm IP đầu dải bằng cách ép toàn bộ 10 bit phần Host còn lại quay về số 0:
  * [10.10.0000.10] [00] . [0000.0000] (22 bit Net Part - 10 bit Host Part)
* **Bước 3:** Biên dịch ngược lại hệ thập phân từng Octet:
  * Octet thứ 3: Cụm bit 0000.1000 -> Vị trí bit số 3 (2^3) mang số 1 ➔ Đạt giá trị thập phân là **8**.
...
* **Bước 1 (Tính tổng nhu cầu):** Ngoài 15 địa chỉ gán cho máy tính nhân viên, hệ thống luôn luôn phải trích thêm 2 địa chỉ chết cố định bắt buộc (gồm 1 IP làm địa chỉ mạng Network ID và 1 IP làm địa chỉ quảng bá Broadcast IP).
  * **Tổng lượng IP tối thiểu hệ thống cần tiêu tốn** = 15 + 2 = **17 địa chỉ IP**
* **Bước 2 (Áp dụng khối lũy thừa cơ số 2):**
  * *Nếu chọn dải bit Host Part = 4:* Khối IP sở hữu là 2^4 = 16 địa chỉ -> Trừ 2 địa chỉ chết còn lại 14 IP ➔ **Không đủ** cấp cho 15 nhân sự.
  * *Nếu chọn dải bit Host Part = 5:* Khối IP sở hữu là 2^5 = 32 địa chỉ -> Trừ 2 địa chỉ chết còn lại 30 IP khả dụng ➔ **Thỏa mãn** chứa được dải 15 máy tính nhân viên và dư thừa chỗ trống cho mở rộng phòng ban sau này.
* **Bước 3 (Tính toán thông số Prefix):**
  * **Prefix** = 32 - Số bit Host Part = 32 - 5 = **27**
